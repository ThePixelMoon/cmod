...
g_SWEPMenu.ExcludeFolders = {".", "..", "football", "build", "hideandseek"}
...
if (g_SWEPMenu.Enabled) then
    ...
    function ProcessSWEPFolder(k, name)
        ...
        for i = 1, table.getn(g_SWEPMenu.ExcludeFolders) do
            if (g_SWEPMenu.ExcludeFolders[i] == name) then return; end
        end
    ...
    function PlayerSendSWEPMenu(iPlayer)
        ...
        for i = 1, table.getn(g_SWEPs) do
            ...
            if (LastCat ~= g_SWEPs[i].category and g_SWEPMenu.ShowCategories) then
                _spawnmenu.AddItem(iPlayer, g_SWEPMenu.MenuTitle,
                                   "@" .. g_SWEPs[i].category, "");
    ...
    function cc_SpawnSWEP(iPlayer, strFilename)
        ...
        -- Time between spawns.
        if (g_SWEPMenu.Players[iPlayer].NextSpawn > _CurTime() and _MaxPlayers() >
            1) then return; end

        if (_GetRule("AllowObjectSpawning") == false) then return end
        if (_file.Exists("lua/weapons/" .. strFilename .. ".lua") == false) then
            return
        end
        ...
        local bFound = false;
        for i = 1, table.getn(g_SWEPs) do
            if (g_SWEPs[i].filename == strFilename) then
                bFound = true;
            end
        end
        ...
        if (_TraceHit() == false) and _PlayerIsKeyDown(iPlayer, IN_SPEED) then
            return
        end
        ...
        if _PlayerIsKeyDown(iPlayer, IN_SPEED) then
            _EntSetPos(iEnt, vSpawnPos)
        else
            _EntSetPos(iEnt, vecAdd(_EntGetPos(iPlayer), vector3(0, 0, 50)))
        end
        ...
        g_SWEPMenu.Players[iPlayer].NextSpawn = _CurTime() +
                                                    g_SWEPMenu.SpawnRate;

    end

    PopulateSWEPList();
    HookEvent("eventPlayerInitialSpawn", PlayerSendSWEPMenu);
    CONCOMMAND("SWEPSpawn", cc_SpawnSWEP,
               "Spawn SWEP weapon; Syntax: <filename>");
