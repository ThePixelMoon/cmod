        ...
        if ((bEndGame == 0) and (RestartRound == 0)) then
            ...
            if (teams[team].lastLevel == (level - 1)) then
                ...
                if (teams[team].padCount == _TeamNumPlayers(team)) then
                    ...
                    local scrmsg = teams[team].name .. " made it to level " ..
                                       level .. ".\n";

                    _ScreenText(0, scrmsg, -1, 0.2, 255, 100, 25, 255, 1, 3, 6,
                                1, 1);
                    ...
                    teams[team].levelsTime[level] =
                        (_CurTime() - roundStartTime);
                    UpdateScores(0);
                    ...
                    if (level == table.getn(levels)) then
                        if (scorelimit > 0) and (_TeamScore(team) >= scorelimit) then
                            ...
                            _ScreenText(0,
                                        "Game over. Score limit has been reached",
                                        -1, 0.15, 0, 0, 255, 255, 1, 3, 6, 1, 3);
                            displayWhoWon();
                            bEndGame = _CurTime() +
                                           _GetConVar_Float("mp_chattime");
                        else
                            _ScreenText(0,
                                        "Round over. " .. teams[team].name ..
                                            " made it to the last level", -1,
                                        0.15, 0, 0, 255, 255, 1, 3, 6, 1, 3);
                            RestartRound = _CurTime() +
                                               _GetConVar_Float("mp_chattime");
                        end
                    end

                    if (levels[level].reward == 1) then
                        messageInfo(0, 74, 0.4, teams[team].name ..
                                        " has been given the gravity freeze upgrade");
                        reward(team, level);
                    elseif (levels[level].reward == 2) then
                        messageInfo(0, 74, 0.4, teams[team].name ..
                                        " can now spawn rpops ontop objects");
                        reward(team, level);
                    end
                else
                    -- all the players aren't on the pad. issue a warning
                    _PrintMessage(activator, 4,
                                  "You need all of your team on the pad");
                end
            else
                -- not at the right level. check to see what they should do
                if (teams[team].lastLevel == table.getn(levels)) then
                    _PrintMessage(activator, 4,
                                  "You have completed all levels. Wait for time limit to expire");
                elseif (teams[team].lastLevel >= level) then
                    _PrintMessage(activator, 4,
                                  "You have already activated this level. You need to build upward.");
                elseif (teams[team].lastLevel < level) then
                    _PrintMessage(activator, 4,
                                  "You have not activated a previous level. You need to go back.");
                end
            end
        end
    end
        ...
        if ((bEndGame == 0) and (RestartRound == 0)) then
            ...
            if (teams[team].lastLevel == (level - 1)) then
                ...
                if (teams[team].padCount == _TeamNumPlayers(team)) then
                    ...
                    elseif (levels[level].reward == 2) then
                        messageInfo(0, 74, 0.4, teams[team].name ..
                                        " can now spawn props ontop objects");
                        reward(team, level);
                    end
                else
                    -- all the players aren't on the pad. issue a warning
...
function eventPlayerKilled(killed, attacker, weapon) _PlayerAddDeath(killed, 1); end
...
function onGravGunDrop(player, entity)
    if ((FREEZE_ON_GRAVGUN_DROP) or
        (teams[_PlayerInfo(player, "team")].reward > 0)) then
        if (_PlayerInfo(playerid, "team") == TEAM_BLUE) then
            _EntFire(entity, "color", "50 150 255", 0);
        elseif (_PlayerInfo(player, "team") == TEAM_YELLOW) then
            _EntFire(entity, "color", "255 200 0", 0);
