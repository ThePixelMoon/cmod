function onShowHelp(userid)
    ...
    message(userid, 51, 0.25, 0.30, 20,
            "The object of this game is to build across to the last level");
    message(userid, 52, 0.25, 0.33, 20,
            "You can use the prop gun (slot 2, 1st gun) to create crates or barrels (alt fire)");
    message(userid, 53, 0.25, 0.36, 20,
            "Use the gravity gun to position the object and have someone else freeze it for you");
    message(userid, 54, 0.25, 0.39, 20,
            "The physgun's alt fire will freeze or use the freeze gun (slot 2, 3rd gun)");
    message(userid, 55, 0.25, 0.42, 20,
            "Certain levels will give you gun upgrades");

    messageMenu(userid, 56, 0.20, 0.47, 20, "gm_build_tothetop:");
    message(userid, 57, 0.25, 0.52, 20,
            "Same rules as gm_build_bridge except you build upwards instead of across");

    messageAttn(userid, 58, 0.20, 0.57, 20,
...
function UpdateScores(UserID)
    ...
    local COLUMN1, COLUMN2, COLUMN3, COLUMN4, COLUMN5 = 0.03, 0.14, 0.17, 0.22,
                                                        0.27;
    ...
    for iTeam in teams do
        ...
        for i in teams[iTeam].levelsTime do
            _GModText_Start("DefaultShadow");
            _GModText_SetPos(COLUMN3 + (col_multi * COL_SPACING),
                             TOP_MARGIN + (marg_multi * LINE_SPACING));
