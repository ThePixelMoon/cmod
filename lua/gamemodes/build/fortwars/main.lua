...
function gamerulesThink()
    ...
    if (DEATH_MATCH) and (fDeathmatch_time <= _CurTime()) and (bEndGame == 0) and
        (RestartRound == 0) and (not dm_mode) then
        messageInfo(0, 67, 0.3, "Deathmatch mode has begun!");
        StartDeathmatch();
    end

    if (DEATH_MATCH) and (fDeathmatch_len <= _CurTime()) and (bEndGame == 0) and
        (RestartRound == 0) and (dm_mode) then
        removeAllPlayersWeapons();
        if (scorelimit > 0) and ((_TeamScore(TEAM_BLUE) >= scorelimit) or
            (_TeamScore(TEAM_YELLOW) >= scorelimit) or
            (_TeamScore(TEAM_GREEN) >= scorelimit) or
            (_TeamScore(TEAM_RED) >= scorelimit)) then
...
function EndDeathmatch()
    ...
    dm_mode = false;
    fDeathmatch_time = _CurTime() + sDeathmatch_time;
    if DEATH_MATCH then tDeathmatch = AddTimer(1, 0, doCountDeathmatch); end
end
...
function StartDeathmatch()
    ...
    if DEATH_MATCH then tDeathmatch = AddTimer(1, 0, doCountDMLen); end
end
...
function GiveDefaultItems(playerid)
    if (_PlayerInfo(playerid, "alive")) and
        (_PlayerInfo(playerid, "team") ~= TEAM_SPECTATOR) then
...
function displayWhoWon()
    if (_TeamScore(TEAM_BLUE) > _TeamScore(TEAM_YELLOW)) and
        (_TeamScore(TEAM_BLUE) > _TeamScore(TEAM_GREEN)) and
        (_TeamScore(TEAM_BLUE) > _TeamScore(TEAM_RED)) then
        -- blue team won
        messageInfo(0, 68, 0.2, "Blue team won the game!");
    elseif (_TeamScore(TEAM_YELLOW) > _TeamScore(TEAM_BLUE)) and
        (_TeamScore(TEAM_YELLOW) > _TeamScore(TEAM_GREEN)) and
        (_TeamScore(TEAM_YELLOW) > _TeamScore(TEAM_RED)) then
        -- yellow team won
        messageInfo(0, 68, 0.2, "Yellow team won the game!");
    elseif (_TeamScore(TEAM_GREEN) > _TeamScore(TEAM_BLUE)) and
        (_TeamScore(TEAM_GREEN) > _TeamScore(TEAM_YELLOW)) and
        (_TeamScore(TEAM_GREEN) > _TeamScore(TEAM_RED)) then
        -- green team won
        messageInfo(0, 68, 0.2, "Green team won the game!");
    elseif (_TeamScore(TEAM_RED) > _TeamScore(TEAM_BLUE)) and
        (_TeamScore(TEAM_RED) > _TeamScore(TEAM_GREEN)) and
        (_TeamScore(TEAM_RED) > _TeamScore(TEAM_YELLOW)) then
        -- red team won
...
function cc_removeAllProps(fromplayer, args)
    if (isAdmin(fromplayer)) then
        ...
        message(0, 99, -1, 0.3, 3,
                _PlayerInfo(fromplayer, "name") .. " removed all props.")
    end
end

function cc_restartRound(fromplayer, args)
    if (isAdmin(fromplayer)) then
        message(0, 99, -1, 0.3, 3, "Round over. " ..
                    _PlayerInfo(fromplayer, "name") .. " restarted the round")
...
function cc_enablePhysgunPickup(fromplayer, args)
    if (isAdmin(fromplayer)) then
        if (args == "1") then
            ENABLE_PHYSGUN_PICKUP = true;
            message(0, 99, -1, 0.3, 3, _PlayerInfo(fromplayer, "name") ..
                        " enabled physgun pickup");
        elseif (args == "0") then
            ENABLE_PHYSGUN_PICKUP = false;
            message(0, 99, -1, 0.3, 3, _PlayerInfo(fromplayer, "name") ..
                        " disabled physgun pickup");
        end
    end
end
...
function cc_enableFreezeGravGunDrop(fromplayer, args)
    if (isAdmin(fromplayer)) then
        if (args == "1") then
            FREEZE_ON_GRAVGUN_DROP = true;
            message(0, 99, -1, 0.3, 3, _PlayerInfo(fromplayer, "name") ..
                        " enabled gravgun freezing");
        elseif (args == "0") then
            FREEZE_ON_GRAVGUN_DROP = false
            message(0, 99, -1, 0.3, 3, _PlayerInfo(fromplayer, "name") ..
                        " disabled gravgun freezing");
        end
    end
end
...
function cc_spawnPropGround(fromplayer, args)
    if (isAdmin(fromplayer)) then
        if (args == "1") then
            ...
            message(0, 99, -1, 0.3, 3, _PlayerInfo(fromplayer, "name") ..
                        " enabled spawning of props on ground only");
        elseif (args == "0") then
            SPAWN_PROPS_GROUND_ONLY = false
            message(0, 99, -1, 0.3, 3, _PlayerInfo(fromplayer, "name") ..
                        " disabled spawning of props on ground only");
        end
    end
end
...
function cc_setScoreLimit(fromplayer, args)
    if (isAdmin(fromplayer)) then
        if tonumber(args) then scorelimit = args; end
    end
end
...
function cc_setDeathmatchMode(fromplayer, args)
    if (isAdmin(fromplayer)) then
        if (args == "1") then
            if dm_mode then
                messageInfo(fromplayer, 85, 0.25,
                            "Error: deathmatch mode is already on");
            else
        ...
        elseif (args == "0") then
            if dm_mode then
                ...
                messageInfo(fromplayer, 85, 0.25,
                            "Error: deathmatch mode is already off");
            end
        end
    end
end
...
function cc_toggleWalls(fromplayer, args)
    if (isAdmin(fromplayer)) then toggleWalls(); end
end
