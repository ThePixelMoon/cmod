function onShowSpare2(userid)
    if (_PlayerInfo(userid, "team") ~= TEAM_SPECTATOR) then buyMenu(userid); end
end

function onShowHelp(userid)
    messageMenu(userid, 50, 0.30, 0.30, 20, "gm_build_fortwars:");
    message(userid, 51, 0.35, 0.35, 20,
            "The object of this game is for each team to build a fort");
    message(userid, 52, 0.35, 0.38, 20,
            "You can use the prop gun (slot 2, 1st gun) to create crates to build with");
    message(userid, 53, 0.35, 0.41, 20,
            "You can use the gravity gun or physgun to position the crates with");
    message(userid, 55, 0.35, 0.44, 20,
            "After bulid mode is over deathmatch begins");
    message(userid, 56, 0.35, 0.47, 20,
            "Grab the basketball that falls in the center with your gravity gun");
    message(userid, 57, 0.35, 0.50, 20,
            "For each minute you hold it, your team gets a point");

    message(userid, 60, 0.35, 0.55, 20,
            "The spawnpoint gun will set where you will be spawned (slot 3, first gun)");
    message(userid, 61, 0.35, 0.58, 20,
            "This gun can only be used during build mode.");
    message(userid, 62, 0.35, 0.61, 20,
            "Note that this gun will set your spawn point to where you standing");
    message(userid, 63, 0.35, 0.64, 20,
            "It does not set your spawnpoint to where you are aiming");
    message(userid, 64, 0.35, 0.67, 20,
            "Alt-fire will reset the spawnpoint to default");

    messageAttn(userid, 58, 0.30, 0.72, 20,
                "Press F2 to change teams. F3 is the admin panel for those with access.");
...
function ChooseTeam(playerid, num, seconds)
    ...
    if players[playerid].canSwitchTeam then
        ...
        if (num == 1) then
            ...
            if (_PlayerInfo(playerid, "team") == TEAM_SPECTATOR) then
                if (_TeamNumPlayers(TEAM_BLUE) > _TeamNumPlayers(TEAM_YELLOW)) or
                    (_TeamNumPlayers(TEAM_BLUE) > _TeamNumPlayers(TEAM_GREEN)) or
                    (_TeamNumPlayers(TEAM_BLUE) > _TeamNumPlayers(TEAM_RED)) then
                    autoJoinTeam(playerid, 0);
                    return;
                end
            else
                if (_TeamNumPlayers(TEAM_BLUE) >=
                    _TeamNumPlayers(_PlayerInfo(playerid, "team"))) then
                    _PrintMessage(playerid, 4,
                                  "Teams would be too uneven if you switched.");
                    return;
                end
            end
        ...
        if (num == 2) then
            ...
            if (_PlayerInfo(playerid, "team") == TEAM_SPECTATOR) then
                if (_TeamNumPlayers(TEAM_YELLOW) > _TeamNumPlayers(TEAM_BLUE)) or
                    (_TeamNumPlayers(TEAM_YELLOW) > _TeamNumPlayers(TEAM_GREEN)) or
                    (_TeamNumPlayers(TEAM_YELLOW) > _TeamNumPlayers(TEAM_RED)) then
                    autoJoinTeam(playerid, 0);
                    return;
                end
            else
                if (_TeamNumPlayers(TEAM_YELLOW) >=
                    _TeamNumPlayers(_PlayerInfo(playerid, "team"))) then
                    _PrintMessage(playerid, 4,
                                  "Teams would be too uneven if you switched.");
                    return;
        ...
        if (num == 3) then
            ...
            if (_PlayerInfo(playerid, "team") == TEAM_SPECTATOR) then
                if (_TeamNumPlayers(TEAM_GREEN) > _TeamNumPlayers(TEAM_YELLOW)) or
                    (_TeamNumPlayers(TEAM_GREEN) > _TeamNumPlayers(TEAM_BLUE)) or
                    (_TeamNumPlayers(TEAM_GREEN) > _TeamNumPlayers(TEAM_RED)) then
                    autoJoinTeam(playerid, 0);
                    return;
                end
            else
                if (_TeamNumPlayers(TEAM_GREEN) >=
                    _TeamNumPlayers(_PlayerInfo(playerid, "team"))) then
                    _PrintMessage(playerid, 4,
                                  "Teams would be too uneven if you switched.");
                    return;
        ...
        if (num == 4) then
            ...
            if (_PlayerInfo(playerid, "team") == TEAM_SPECTATOR) then
                if (_TeamNumPlayers(TEAM_RED) > _TeamNumPlayers(TEAM_YELLOW)) or
                    (_TeamNumPlayers(TEAM_RED) > _TeamNumPlayers(TEAM_BLUE)) or
                    (_TeamNumPlayers(TEAM_RED) > _TeamNumPlayers(TEAM_GREEN)) then
                    autoJoinTeam(playerid, 0);
                    return;
                end
            else
                if (_TeamNumPlayers(TEAM_RED) >=
                    _TeamNumPlayers(_PlayerInfo(playerid, "team"))) then
                    _PrintMessage(playerid, 4,
                                  "Teams would be too uneven if you switched.");
                    return;
        ...
        messageInfo(playerid, 76, 0.25,
                    "You can't switch teams until next round");
    end
end

function UpdateScores(UserID)

    local COLUMN1, COLUMN2, COLUMN3, COLUMN4, COLUMN5 = 0.03, 0.14, 0.19, 0.24,
                                                        0.27;
    ...
    for iTeam in teams do
        ...
        _GModText_SetColor(teams[iTeam].color.r, teams[iTeam].color.g,
                           teams[iTeam].color.b, 255);
        _GModText_SetTime(99999, 0, 0);
