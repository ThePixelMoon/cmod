...
function gamerulesThink()
    if (bEndRound) then
        if (fIntermissionEnd < _CurTime()) then RoundRestart(); end
        return;
    end
            ...
            if (zombies[i]) then
                ...
                if (not sethealth[i]) then
                    ...
                    if MULTIPLY_ZOMBIE_HEALTH then
                        local addHealth = (ZOMBIE_HEALTH *
                                              (ihumanCount - iZombieCount));
                        if (addHealth > 0) then
                            if _PlayerInfo(i, "alive") then
    ...
    if (humanCount == 1) and (playerCount > 2) and (not bEndRound) then
        if (winner > 0) then
            _ScreenText(0, _PlayerInfo(winner, "name") .. " won the game!", -1,
                        0.2, 255, 100, 0, 255, 1, 3, 6, 1, 1);
            _PlayerAddScore(winner, 1);
        end

        StartIntermission();
    elseif ((zombieCount < 1) or (humanCount < 1)) and (playerCount > 1) and
        (not bEndRound) then
        _ScreenText(0, "Restarting the round...", -1, 0.2, 255, 100, 0, 255, 1,
                    3, 6, 1, 1);
        StartIntermission();
    end
end
...
function RoundRestart()
    ...
    for i = 1, _MaxPlayers() do
        if not zombies[i] then
            _PlayerRespawn(i);
        else
            if _PlayerInfo(i, "alive") then
                _PlayerSilentKill(i, 5, true)
            else
                _PlayerRespawn(i);
            end
        end
...
function SwitchToZombie(userid)
    ...
    if not playerZombieModel[userid] then
        ...
        zombieModel = playerZombieModel[userid]
    end
    _PlayerSetModel(userid, zombieModel);
    _PlayerChangeTeam(userid, TEAM_GREEN);
end
...
function SwitchToHuman(userid)
    ...
    if not playerHumanModel[userid] then
        ...
        humanModel = playerHumanModel[userid]
    end
    _PlayerSetModel(userid, humanModel);
    _PlayerChangeTeam(userid, TEAM_BLUE);
end
