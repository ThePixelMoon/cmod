...
function gamerulesStartMap() PlayerFreezeAll(false); end
...
function PlayerDoneLap(iPlayer)
    ...
    if (PlayerInfo[iPlayer].LapTime < PlayerInfo[iPlayer].BestLap or
        PlayerInfo[iPlayer].BestLap == 0) then
        ...
        if (PlayerInfo[iPlayer].BestLap < Stats.BestLap or Stats.BestLap == 0) then
            ...
            Stats.BestLapPrint = ToMinutesSecondsMilliseconds(
                                     PlayerInfo[iPlayer].BestLap);
...
function UpdateTopThree()
    ...
    if (OldFirst ~= Stats.FirstPlace or OldSecond ~= Stats.SecondPlace or
        OldThird ~= Stats.ThirdPlace) then DrawStats(0); end
...
function StartRound()
    ...
    for i = 1, _MaxPlayers() do
        if (IsPlayerOnline(i) and _PlayerInfo(i, "team") ~= TEAM_SPECTATOR) then
...
function RaceStart()
    ...
    for i = 1, _MaxPlayers() do
        ...
        if (IsPlayerOnline(i) and PlayerInfo[i].Melon ~= 0) then
