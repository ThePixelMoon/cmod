...
function eventPlayerSpawn(userid)
    ...
    if (bIntermission == true) then _phys.EnableMotion(iMelon, false); end
...
function MelonToPlayer(Melon)
    ...
    for i = 1, _MaxPlayers() do
        ...
        if (PlayerInfo[i].Melon ~= 0 and PlayerInfo[i].Melon == Melon) then
...
function eventKeyPressed(userid, in_key)
    ...
    if (in_key == IN_ATTACK and _PlayerInfo(userid, "team") == TEAM_SPECTATOR) then
