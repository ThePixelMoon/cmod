...
function PlayerShatOn(iThisPlayer, iOtherPlayer)
    ...
    _EntEmitSound(PlayerData[iThisPlayer].Bird,
                    ...
                    if (hitEnt ~= iPlayer) then
                        if (_PlayerInfo(hitEnt, "connected")) then
                            ...
                            for key in PlayerData do
                                if (_PlayerInfo(key, "connected") and
                                    _PlayerInfo(key, "team") == TEAM_BIRDS) then
...
function DoSpeed(i)
    ...
    if (PlayerData[i].IsSpeedFiring == 0) then
        if (PlayerData[i].RunLeft < MAX_RUN) then return end
    end
...
function DoUpdateTarget(i)
    ...
    if (i == nil or PlayerData[i] == nil) then return end
        ...
        if (_PlayerInfo(i, "team") == TEAM_BIRDS) then
            ...
            if (_PlayerIsKeyDown(i, IN_DUCK)) then
                ...
                if (PlayerData[i].IsSpeedFiring == 0) then
                    ...
                    vVel = vecMul(vAimPos,
                                  vector3(FAST_WALK_SPEED, FAST_WALK_SPEED, 0))
                    _EntSetActivity(PlayerData[i].Bird, ACT_RUN);
                end
            else
                if (PlayerData[i].IsSpeedFiring == 0) then
                    vVel = vecMul(vAimPos, vector3(NORMAL_SPEED, NORMAL_SPEED,
                                                   NORMAL_SPEED))
                    _EntSetActivity(PlayerData[i].Bird, ACT_FLY);
                else
                    vVel = vecMul(vAimPos,
                                  vector3(FAST_SPEED, FAST_SPEED, FAST_SPEED))
                    _EntSetActivity(PlayerData[i].Bird, ACT_FLY);
                end
            end
