...
function ChooseBirdType(playerid, num, seconds)
    ...
    elseif (num == 2) then
        PlayerData[playerid].BirdType = BT_CROW
    elseif (num == 3) then
        PlayerData[playerid].BirdType = BT_PIGEON
    else
    ...
    -- nothing to change!
    if (PlayerData[playerid].BirdType == oldBirdType and
        _PlayerInfo(playerid, "team") == TEAM_BIRDS) then return end

    if (PlayerData[playerid].BirdType == BT_SEAGULL) then
        _GModRect_Hide(playerid, 6, 1.5);
    elseif (PlayerData[playerid].BirdType == BT_CROW) then
        _GModRect_Hide(playerid, 7, 1.5);
    elseif (PlayerData[playerid].BirdType == BT_PIGEON) then
        _GModRect_Hide(playerid, 8, 1.5);
    end
...
function onShowSpare1(userid) cc_menu(userid) end
