function eventPlayerKilled(killed, attacker, weapon)
    ...
    if (PlayerData[killed] ~= nil) then ClearPlayerData(killed, 1) end
...
function eventPlayerSpawn(userid)
    ...
    if (_PlayerInfo(userid, "team") == TEAM_BIRDS) then
        ...
        PlayerData[userid].UpdateTimer =
            AddTimer(0.1, 0, DoUpdateTarget, userid);
...
function eventNPCKilled(killerid, killed)
    ...
    for key in PlayerData do
        ...
        if (_PlayerInfo(key, "connected") and _PlayerInfo(key, "team") ==
            TEAM_BIRDS) then
            ...
            if (PlayerData[key].Bird == killed) then
                ...
                if (_PlayerInfo(killerid, "connected")) then
                    ...
                    _GModText_SetText("You were killed by " ..
                                          _PlayerInfo(killerid, "name") .. "!");
                    ...
                    _GModText_SetPos(-1, 0.2);
                    _GModText_SetText(
                        "You killed " .. _PlayerInfo(key, "name") .. "!");
