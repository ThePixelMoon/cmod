...
function ClearPlayerData(playerid, respawn)
    ...
    if (PlayerData[playerid] == nil) then return end
    ...
    if (respawn ~= 0) then AddTimer(3, 1, _EntSpawn, playerid); end
end
...
function AddHUDPoop(playerid)
    ...
    if (PlayerData[playerid].LastPoopDecal == 0) then
        ...
        PlayerData[playerid].LastPoopDecal =
            PlayerData[playerid].LastPoopDecal + 1

        if (PlayerData[playerid].LastPoopDecal >
            (MAX_HUD_POOP + FIRST_HUD_POOP_DECAL)) then
            PlayerData[playerid].LastPoopDecal = FIRST_HUD_POOP_DECAL
        end
    end
