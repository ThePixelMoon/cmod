...
function gamerulesThink()
    ...
    if (bEndGame) then
        ...
        if (fIntermissionEnd < _CurTime()) then _StartNextLevel() end
    ...
    if (FragLimit > 0) then -- We have a fraglimit!
        ...
        if (_GetRule("Teamplay")) then
            ...
            for i = 0, _MaxPlayers() do
                ...
                if (_PlayerInfo(i, "connected") and _PlayerInfo(i, "kills") >=
                    FragLimit) then StartIntermission() end
    ...
    if (TimeLimit > 0) then
        ...
        if (TimeLimit < _CurTime()) then StartIntermission() end
...
function cc_thebest(fromplayer)
    _ScreenText(fromplayer,
                "The best sniper is " .. longestperson .. " with " ..
                    longestdist .. " feet", -1, -.35, 255, 0, 0, 255, 0.1, 1.0,
                3, 1, 4)
...
function cc_worst(fromplayer)
    _ScreenText(fromplayer,
                "The worst sniper is " .. shortestperson .. " who got shot at " ..
                    shortestdist .. " feet. HAHAHAHAHA!", -1, -.27, 255, 0, 0,
                255, 0.1, 1.0, 3, 1, 3)
end
function cc_therecords(fromplayer)
    ...
    if hisid == "UNKNOWN" then
        ...
        for i = 1, _MaxPlayers() do
            _Msg(
                "Sending the stats to all players, don't worry about the errors if there are any.\n")
            _GModRect_Send(i, 0);
        end
    end
end
...
function eventPlayerKilled(killed, attacker, weapon)
    ...
    elseif (attacker > 0) then
        ...
        _ScreenText(killed,
                    _PlayerInfo(attacker, "name") .. " killed you from " ..
                        thelength .. " feet", -1, -.75, 255, 0, 0, 200, 0.1,
                    1.0, 3, 1, 0)
        _ScreenText(attacker,
                    "You killed " .. _PlayerInfo(killed, "name") .. " from " ..
                        thelength .. " feet", -1, -.75, 255, 0, 0, 200, 0.1,
                    1.0, 3, 1, 0)
        if thelength > longestdist then
            ...
            for i = 1, _MaxPlayers() do
                _ScreenText(i,
                            _PlayerInfo(attacker, "name") ..
                                " got the new longest shot with  " .. thelength ..
                                " feet", -1, -.5, 255, 0, 0, 255, 0.1, 1.0, 3,
                            1, 5)
            end
        end
        if thelength < shortestdist then
            ...
            for i = 1, _MaxPlayers() do
                _ScreenText(i,
                            _PlayerInfo(killed, "name") ..
                                " got shot at the shortest range of  " ..
                                thelength .. " feet. HAHAHAHAHAH!", -0.68, 0,
                            255, 0, 0, 255, 0.1, 1.0, 3, 1, 5)
            end
        end
