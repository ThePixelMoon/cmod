function gamerulesThink()
    ...
    if (_CurTime() >= roundEndTime) then
        ...
        if (roundEndFunc ~= nil) then roundEndFunc() end
...
function ShowWall()
    if (wallEnabled == 1) then return end

    local wall = _EntGetByName("Wall")
    if (wall ~= 0) then _EntFire(wall, "Toggle", 0, 0) end
    wallEnabled = 1
end

function HideWall()
    if (wallEnabled == 0) then return end
...
function BuildMode()
    ...
    if (rtChanged) then PlayerSpawnAll() end
end

function restartRound()
    if (roundType == RT_BUILD) then
        BuildMode()
    elseif (roundType == RT_FLYTEST) then
        StartFlyTestMode()
    elseif (roundType == RT_DEATHMATCH) then
        BeginDeathMatchMode()
    end
...
function EndDeathMatchMode()
    ...
    for i = 1, _MaxPlayers() do _PlayerSilentKill(i) end
...
function SetNextRoundDeathMatch() roundEndFunc = BeginDeathMatchMode end

function canPlayerHaveItem(playerid, itemname)

    if ((itemname == "weapon_physcannon" or itemname == "weapon_physgun" or
        itemname == "weapon_Tool") and roundType == RT_DEATHMATCH) then

        return false

    elseif ((itemname ~= "weapon_physcannon" and itemname ~= "weapon_physgun" and
        itemname ~= "weapon_Tool") and roundType ~= RT_DEATHMATCH) then
