...
function gamerulesThink()
    ...
    elseif ((_TeamNumPlayers(TEAM_HIDERS) < 1) and (HSNumFound > 0)) then
        ...
        _PrintMessageAll(3,
                         "All Hiders have been found. A new round begins in a few seconds.");

        if (_TeamScore(TEAM_HIDERS) > _TeamScore(TEAM_SEEKERS)) then
            _PrintMessageAll(3, "The Hiders won that round with " ..
                                 _TeamScore(TEAM_HIDERS) .. " points! ");
        elseif (_TeamScore(TEAM_HIDERS) < _TeamScore(TEAM_SEEKERS)) then
            _PrintMessageAll(3, "The Seekers won that round with " ..
                                 _TeamScore(TEAM_SEEKERS) .. " points!");
        else
            _PrintMessageAll(3, "Both teams tied with " ..
                                 _TeamScore(TEAM_HIDERS) .. " points!");
        end
    ...
    for userid = 0, _MaxPlayers() do
        if (_PlayerInfo(userid, "connected") and
            ((_PlayerInfo(userid, "team") == TEAM_HIDERS) or
                (_PlayerInfo(userid, "team") == TEAM_SEEKERS)) and
            (_PlayerInfo(userid, "alive")) and
            (_GetCurrentMap() == "gm_hideandseek")) then
            local vecpos = _PlayerGetShootPos(userid);
            if (vecpos.z < -500) then
                _PrintMessage(userid, 3,
                              "Sorry, can't let you down there, it's too cold!");
...
function eventPlayerSpawn(userid)
    ...
    if (HSIsFound[userid]) then
        ...
        if (_TeamNumPlayers(TEAM_HIDERS) > 1) then
            _PrintMessage(userid, 3,
                          "You are spectating. Please wait until the round ends.");
        end
    ...
    elseif (_PlayerInfo(userid, "team") == TEAM_SEEKERS) then
        ...
        AddTimer(0.01, 1, _PlayerGiveSWEP, userid,
                 "weapons/hideandseek/weapon_supercatcher.lua");
        AddTimer(0.02, 1, _PlayerGiveSWEP, userid,
                 "weapons/hideandseek/weapon_catcher.lua");

    else

        _PrintMessage(userid, 3,
                      "You have spawned as a Hider. Hide for as long as possible.");
        _PrintMessage(userid, 3,
                      "You have been granted invisibility for 30 seconds. Use this time to hide!");
...
function eventPlayerActive(name, userid, steamid) DrawScores(userid); end
