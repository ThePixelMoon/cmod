...
function gamerulesThink() DoTimers() end
...
function eventPlayerKilled(killed, attacker, weapon)
    _PrintMessageAll(HUD_PRINTCENTER,
                     "How did " .. _PlayerInfo(killed, "name") .. " die?")
end

function PickDefaultSpawnTeam(userID) ChooseTeam(userID, 5) end
...
function PlayerSpawnChooseModel(userid)
    ...
    if (teamID == 2 or teamID == 3) then
        ...
        _EntFire(_PlayerInfo(userid, "entindex"), "color",
                 Team[teamID].Model.Color.Default, 0)
...
function giveAllWeapons()
    for i = 0, _MaxPlayers() do
        if (_PlayerInfo(i, "connected")) then GiveDefaultItems(i) end
    end
end
...
function setNotInGoalie(activator, caller, teamID) end
    ...
    if (IsPlayer(activator)) then
        ...
        if (aTeam == pTeam) then
            if (not Team[pTeam].Goalie) then
                ...
                if (Team[pTeam].Model.Color.Goalie) then
                    _EntFire(activator, "color", Team[pTeam].Model.Color.Goalie,
                             0)
                end
                if (Team[pTeam].Model.Goalie) then
                    _PlayerSetModel(activator, Team[pTeam].Model.Goalie)
                end
    ...
    if (IsPlayer(activator)) then
        ...
        if (pTeam == aTeam) then
            if (IsGoalie(activator)) then
                ...
                if (Team[pTeam].Model.Color.Default) then
                    _EntFire(activator, "color",
                             Team[pTeam].Model.Color.Default, 0)
                end
                if (Team[pTeam].Model.Default) then
                    _PlayerSetModel(activator, Team[pTeam].Model.Default)
                end
...
function BallHit(activator, caller) Ball.lasthit = activator end
...
function UpdateUI(userID)
    ...
    _GModText_SetText(Team[TEAM_YELLOW].Goals .. "\n" ..
                          ...
                          ((Team[TEAM_YELLOW].Goals * 10) -
                              (Team[TEAM_YELLOW].Penalties * 5)) .. "\n" ..
...
function onGravGunPunt(userID, entid)
    if (entid == Ball.entid) then BallHit(userID, userID) end
    return true;
end
