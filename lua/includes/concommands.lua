...
function CONCOMMAND(command, funct, help) gConsoleCommands[command] = funct; end
...
function cc_luacommands(playerID, args)
    ...
    table.foreach(gConsoleCommands, function(k, v)
        _PrintMessage(playerID, HUD_PRINTCONSOLE, k .. "\n")
    end);
