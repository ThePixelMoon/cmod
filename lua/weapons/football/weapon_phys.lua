...
function onPrimaryAttack()
    ...
    if hasBall then
        ...
        if (ball > 0) then
            ...
            local spawnpos = vecAdd(_PlayerGetShootPos(Owner),
                                    _PlayerGetShootAng(Owner));
            _EntSetPos(ball, spawnpos);
            _EntSetAng(ball, _PlayerGetShootAng(Owner));

            local fireforce = vecMul(_PlayerGetShootAng(Owner),
                                     vector3(power, power, power));
            _EntSpawn(ball);
    ...
    if (_TraceHit() == true) then
        if (_TraceHitNonWorld()) and (_TraceGetEnt() > _MaxPlayers()) and
            (_TraceGetEnt() == _EntGetByName("theball01")) then
...
function getViewModel() return "models/weapons/v_physcannon.mdl"; end

function getWorldModel() return "models/weapons/w_physics.mdl"; end

function getClassName() return "weapon_football_cannon"; end
...
function getAnimPrefix() return "phys"; end

function getPrintName() return "Football Cannon"; end
